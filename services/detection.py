...
class detection():
    ...
    def seg_color(self,im):
        ...
        if self.use_h_only==0:
            label=(h>self.H_low_thresh)*(h <self.H_high_thresh)*(s>self.S_low_thresh)*(v>self.V_low_thresh)
        else:
    ...
    def det_circle(self,img):
        ...
        # circles=cv2.HoughCircles(gray,cv2.cv.CV_HOUGH_GRADIENT,1,100,param1=120,param2=40,minRadius=1,maxRadius=40)
        # for opencv3
        circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 100, param1=120, param2=40, minRadius=1, maxRadius=160)
        res_circles=[]
...
#        print(len(rects))
        if len(rects)==0:
            ...
            for i in range(0,len(rects)):
                ...
                xmax=rect[2]
                ymax=rect[3]
                if ymax-ymin<200 and xmax-xmin<200:
        ...
        if len(circles)!=0 and len(rects)!=0:
            for i in range(0,len(circles)):
                for j in range(0,len(rects)):
                    ...
                    if inter_w>0 and inter_h>0 and abs((2*inter_w*inter_h*1.0)/(4*r*r+rect_w*rect_h)-1)<0.3:
